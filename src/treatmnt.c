...
//
{
    ...
    if ( Node[j].treatment == NULL && Node[j].externalTreatment == 0 ) return;
    ErrCode = 0;
    J  = j;                            // current node
	...
	else if ( Node[j].externalTreatment == 1 && q <= ZERO ) R[p] = 0.0;

        // --- no removal for removal-type expression when there is no inflow 
	else if ( treatment->treatType == REMOVAL && q <= ZERO && Node[j].externalTreatment == 0 ) R[p] = 0.0;
    ...
    {
        if ( R[p] == 0.0 && Node[j].externalTreatment == 0 ) continue;
        treatment = &Node[j].treatment[p];

        // --- removal-type treatment equations get applied to inflow stream
        if ( treatment->treatType == REMOVAL && Node[j].externalTreatment == 0  )
        {
	...
	else if ( Node[j].externalTreatment == 1 ) cOut = Node[j].externalQual[p];
